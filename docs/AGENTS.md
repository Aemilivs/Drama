...
- **An external model class enters through the executor seam, never as a dependency.** A new kind of model — e.g. a calibrated decision model — becomes a host-supplied function plus (at most) an actor kind, not a provider client and not a "supported X" list. Report the smallest seam and the explicit non-goals, as [`decision-models.md`](decision-models.md) does for System One models.
